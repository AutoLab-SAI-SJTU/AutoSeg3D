    ...
    def _select_queries(self, x, gt_instances, sp_xyz=None):
        ...
        for i in range(len(x)): # batch_size
            if self.query_thr < 1: # 0.5
                n = (1 - self.query_thr) * torch.rand(1) + self.query_thr # 0.5 ~ 1
                n = (n * len(x[i])).ceil().int()
                ids = torch.randperm(len(x[i]))[:n].to(x[i].device) # [N_remained]
                queries.append(x[i][ids]) # [N_remained, 96]
                gt_instances[i].query_masks = gt_instances[i].sp_masks[:, ids] # [N_gts, N_remained]
