...
# 其功能是接收一个二维列表 the_list，并返回一个包含每列最大值的列表
...
# 它的作用是将一个张量列表（tensor_list）转化为一个 NestedTensor 对象，并在转换过程中做适当的填充处理，确保所有图像的尺寸一致（即填充到最大尺寸）
def nested_tensor_from_tensor_list(tensor_list: List[Tensor], size_divisibility: int = 0):
    ...
    if tensor_list[0].ndim == 3:
        ...
        b, c, h, w = batch_shape #[1,3,992,1322]
