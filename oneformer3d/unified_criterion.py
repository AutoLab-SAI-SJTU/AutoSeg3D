...
class ScanNetMixedCriterion:
    ...
    def __call__(self, pred, insts, insts_pts, sp_xyz_list, mask_pred_mode, use_temporal_loss=False, mot_type=None, track_instances=None, matched_dict=None, use_one2many=False):
        """Calculate loss.
        ...
        for i in range(len(pred['masks'])): # batch_size
            sem_gt = InstanceData()
            if insts[i].get('query_masks') is not None:
                sem_gt.sp_masks = insts[i].query_masks[-n - 1:, :]
            else:
            ...
            # point-level gt_mask
            inst_gt = InstanceData()
            inst_gt.p_masks = insts_pts[i].p_masks # [27, 20000]
            inst_gt.sp_masks = insts[i].sp_masks[:-n - 1, :] # [27, 43]
            if pred['cls_preds'][i].shape[1] == 2: # True
                # category agnostic
                inst_gt.labels_3d = torch.zeros_like(insts[i].labels_3d[:-n - 1]) # [27]
            else:
                inst_gt.labels_3d = insts[i].labels_3d[:-n - 1]
            if 'bboxes_3d' in insts[i].keys():
                inst_gt.bboxes_3d = insts[i].bboxes_3d[:-n - 1, :] # [27, 7]
            if insts[i].get('query_masks') is not None:
                inst_gt.query_masks = insts[i].query_masks[:-n - 1, :] # [27, 31]
            inst_gts.append(inst_gt)
        
        if mot_type is None:
            if use_temporal_loss:
                loss, inst_dict = self.inst_criterion(pred, inst_gts, mask_pred_mode, use_temporal_loss=use_temporal_loss)
                loss.update(self.sem_criterion(pred, sem_gts))
                return loss, inst_dict
            else:
                loss = self.inst_criterion(pred, inst_gts, mask_pred_mode, use_temporal_loss, use_one2many=use_one2many)
                loss.update(self.sem_criterion(pred, sem_gts))
                if use_one2many:
                    loss = {f"one2many_{k}": v for k, v in loss.items()}
                return loss
        elif mot_type == 'motr':
            loss, indices = self.inst_criterion(pred, inst_gts, mask_pred_mode, mot_type=mot_type, track_instances=track_instances, matched_dict=matched_dict)  
            loss.update(self.sem_criterion(pred, sem_gts))
