...
model = dict(
    ...
    use_mot=use_mot,
    mot_type=mot_type,
    use_relative_asso=use_relative_asso,
    merge_sp_masks=merge_sp_masks,
...
test_pipeline = [
    ...
    dict(type='Pack3DDetInputs_Online', keys=[
            'points', 'gt_labels_3d', 'pts_semantic_mask', 'pts_instance_mask',
            'sp_pts_mask', 'gt_sp_masks', 'elastic_coords'
        ] + ['gt_bboxes_3d'] +['img_paths'] ,
         dataset_type='scannet')
]
...
default_hooks = dict(
    ...
    checkpoint=dict(type='CheckpointHook', interval=4, max_keep_ckpts=100),
